...
ssh.connect("192.168.1.12", port = 22, username = "cisco", password = "cisco", allow_agent=False,look_for_keys=False) # conenct ot device last 2 vars are to prevent authentication errors



stdin, stdout, stderr = ssh.exec_command("show run") # simple command that doesnt require input from user

#stdin, stdout, stderr = ssh.exec_command("ifconfig") # command that requires further input
